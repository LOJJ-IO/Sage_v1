...
# FEAT: Manage team (accounts)

## Status
`in-progress` — UI at `/manage-team` implemented; backend wiring pending
...
## Solution
Admin-only **Manage team** page at **`/manage-team`** (spec calls this "Manage Accounts" under gear settings):
...
- Gear-icon entry from workspace shell (link/route exists; shell nav not wired)
...
## UI/UX
- Route: `frontend/src/app/manage-team/`
- Components: `ManageTeamView`, `AccountsTable`, `AddAccountDialog`, `ResetPinDialog`, `AccountRowMenu`
...
## Open questions
- Final route name: `/manage-team` (current) vs in-app label "Manage Accounts" from spec — both acceptable; gear menu can label "Manage team" or "Manage accounts"
