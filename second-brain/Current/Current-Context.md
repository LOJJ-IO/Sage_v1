...
- Preview-tabs pure state layer shipped: `frontend/src/lib/preview-tabs/` (types, reducer, selectors, storage, zustand store — first zustand usage and first test runner (vitest) in the repo), 65 passing tests, clean `tsc`/`eslint`. No UI/viewers wired yet. See [[FEAT-preview-tabs]].
...
7. Preview-tabs: wire the pure state layer into the UI — tab-strip component in `page.tsx`'s empty center panel, real file viewers (PDF/docx/image — none exist yet), `markResourceRemoved` triggered by diffing `use-file-library.ts`'s 2s poll against open tabs, and debounced `updateViewState` syncing once a viewer exists. See [[FEAT-preview-tabs]].
