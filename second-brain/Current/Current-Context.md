...
- Preview-tabs pure state layer shipped: `frontend/src/lib/preview-tabs/` (types, reducer, selectors, storage, zustand store — first zustand usage and first test runner (vitest) in the repo), 65 passing tests, clean `tsc`/`eslint`. Phase 7 UI brief written (tab strip only, no viewers). See [[FEAT-preview-tabs]].
...
7. Preview-tabs **Phase 7**: tab-strip UI in center panel (`page.tsx` ~565) wired to `usePreviewTabsStore` — no viewers yet. Full brief: [[FEAT-preview-tabs#Phase 7 — Tab-strip UI implementation brief (no viewers)]]. Then Phase 8: file-tree open, `markResourceRemoved` poll diff, real viewers + debounced `updateViewState`.
