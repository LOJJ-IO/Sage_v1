...
2. If the task touches architecture, check [[Architecture-Overview]] and the relevant [[Architecture/Decisions|ADR]] before proposing a new approach — a past decision may already explain why the "obvious" solution was rejected.
3. If the task touches a known bug or pattern, check [[Known-Issues]] / [[Reusable-Patterns]] first.
4. When you finish non-trivial work, write back: update [[Current-Context]], add/update a bug or ADR or feature spec, and append a line to today's [[Daily]] note.
