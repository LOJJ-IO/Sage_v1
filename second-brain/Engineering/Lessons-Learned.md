...
### 2026-08-04 — Docling's layout model defaults to `torch.compile()`, which hard-fails PDF ingestion without a C++ compiler
Every PDF upload failed outright with `InvalidCxxCompiler: No working C++ compiler found in torch._inductor.config.cpp.cxx` — not a slow cold start, a full ingestion failure (`files.status = "failed"`). Root cause: `docling`'s newer object-detection/image-classification inference engines (used by the PDF layout pass, which runs on every page regardless of content) default `compile_model=True` (`InferenceSettings.compile_torch_models`, `docling/datamodel/settings.py`) — `torch.compile()` needs TorchInductor to JIT via a real C++ toolchain (`g++`), which this dev machine (and, unconfirmed, the deploy image) doesn't have.
**Fix:** `from docling.datamodel.settings import settings as docling_settings; docling_settings.inference.compile_torch_models = False` in `_build_converter()` (`app/ingestion/extract.py`) — it's a plain mutable pydantic singleton, not frozen, so overriding it directly (rather than only via the `DOCLING_INFERENCE_COMPILE_TORCH_MODELS` env var) works regardless of which environment sets what. Verified with a hand-built minimal synthetic PDF: failed with the compiler error before the fix, extracted cleanly after.
**Lesson:** a "performance optimization" default on a heavy ML dependency (torch.compile, in this case) can be a hard failure, not just a missed speedup, on any environment without the toolchain that optimization silently assumes exists — check for buried `compile`/`jit`-flavored settings before trusting a library's out-of-the-box defaults in a container that wasn't built with a full dev toolchain.
